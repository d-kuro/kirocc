...
- **Extended Thinking** — Supports `[1m]` suffix, `thinking` field, `output_config.effort`, and `reasoning_effort` / `budget_tokens` for extended thinking mode
...
2. Derived from `output_config.effort` (or legacy `thinking.reasoning_effort`): `max` = 160000, `high` = 31999, `medium` = 10000, `low` = 4000
