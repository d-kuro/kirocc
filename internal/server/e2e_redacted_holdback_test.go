		...
		for _, stream := range []bool{true, false} {
			...
			t.Run(tt.name+"/"+mode, func(t *testing.T) {
				...
				if stream {
					...
					assertRedactedHoldbackStream(t, string(body), []string{"text"}, tt.text)
					return
				}
...
func TestE2E_RedactedBeforeHoldback(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		},
		{
			...
			events:    slices.Concat(blob, textEvent("<"), blob),
			wantTypes: []string{"redacted_thinking", "text"},
			wantText:  "<",
		},
		{
			name:      "interior blob before held text",
			events:    slices.Concat(textEvent("HEAD"), blob, textEvent("<"), blob),
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			assertRedactedHoldbackStream(t, string(body), tt.wantTypes, tt.wantText)
		})
	}
}

// Validate the client-visible block lifecycle as well as the complete text.
func assertRedactedHoldbackStream(t *testing.T, body string, wantTypes []string, wantText string) {
	t.Helper()
	var types []string
