...
func (s *Service) handleStreamingResponse(ctx context.Context, w http.ResponseWriter, apiResp *kiroclient.Response, model string, contextWindowSize int, stopSequences []string, maxTokens int, preCountedInputTokens int, capture *upstreamAttemptCapture) string {
	...
	// Log response completion and detect truncation (only on success).
	if !streamErr {
		...
		s.storeTruncationIfNeeded(sw.TruncationInput(), sw.ConversationID())
	}
	return ""
}
...
func (s *Service) handleNonStreamingResponse(ctx context.Context, w http.ResponseWriter, apiResp *kiroclient.Response, model string, contextWindowSize int, stopSequences []string, maxTokens int, preCountedInputTokens int, capture *upstreamAttemptCapture) string {
	...
	// Detect truncation.
	s.storeTruncationIfNeeded(stats.TruncationInput, stats.ConversationID)
...
// storeTruncationIfNeeded detects truncation and stores it for the next request.
func (s *Service) storeTruncationIfNeeded(input respconv.TruncationInput, conversationID string) {
	truncType := respconv.DetectTruncation(input)
	if truncType != respconv.TruncationNone && conversationID != "" {
		s.truncStore.Set(conversationID, respconv.TruncationInfo{Type: truncType})
	}
}
