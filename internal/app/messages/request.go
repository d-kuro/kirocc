...
func (s *Service) HandleCountTokens(w http.ResponseWriter, r *http.Request) {
	...
	payload, err := reqconv.BuildPayload(req, reqconv.BuildOptions{ProfileARN: profileARN, ModelID: kiroModel, ConversationID: s.conversationID, Thinking: thinking, ThinkingBudget: 0, EnvState: s.envState})
	if err != nil {
		WriteErrorJSON(w, http.StatusBadRequest, errTypeInvalidRequest, err.Error())
		return
	}
