...
func (s *Service) HandleMessages(w http.ResponseWriter, r *http.Request) {
	...
	payload, err := reqconv.BuildPayload(req, reqconv.BuildOptions{ProfileARN: creds.ProfileARN, ModelID: kiroModel, ConversationID: s.conversationID, Thinking: thinking, ThinkingBudget: thinkingBudget, EnvState: s.envState})
	if err != nil {
	...
	// Inject truncation notice from previous request if available.
	convID := payload.ConversationState.ConversationID
	if info, ok := s.truncStore.Pop(convID); ok {
		notice := respconv.ContentTruncationNotice
		if info.Type == respconv.TruncationTool {
			notice = respconv.ToolTruncationNotice
		}
