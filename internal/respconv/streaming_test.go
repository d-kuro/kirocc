...
func TestSSEWriter_MessageMetadataEvent(t *testing.T) {
	w := httptest.NewRecorder()
	sw := NewSSEWriter(context.Background(), w, "claude-sonnet-4.6", 200000, nil, 0, 0)

	sw.HandleEvent(kiroproto.Event{Type: "messageMetadataEvent", ConversationID: "conv-123", UtteranceID: "utt-456"})
	if sw.ConversationID() != "conv-123" {
		t.Fatalf("conversationID = %q", sw.ConversationID())
	}
}
