...
// NonStreamingStats holds truncation-related stats and token usage from a non-streaming response.
...
// Also returns stats for truncation detection.
...
func buildResponseFromAcc(acc *responseAccumulator, model string) (map[string]any, NonStreamingStats) {
	...
	stats := NonStreamingStats{
		TruncationInput:        acc.TruncationInput(),
