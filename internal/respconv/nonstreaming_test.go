...
func TestBuildNonStreamingResponse_StatsConversationID(t *testing.T) {
	events := []kiroproto.Event{
		{Type: "assistantResponseEvent", Content: "Hi"},
		{Type: "messageMetadataEvent", ConversationID: "conv-abc"},
		{Type: "metadataEvent", InputTokens: 10, OutputTokens: 5},
	}
	_, stats := BuildNonStreamingResponse(events, "claude-sonnet-4.6", 200000, nil, 0, 0)
	if stats.ConversationID != "conv-abc" {
		t.Fatalf("ConversationID = %q", stats.ConversationID)
	}
}

func TestBuildNonStreamingResponse_StatsToolParseError(t *testing.T) {
	events := []kiroproto.Event{
		{Type: "toolUseEvent", ToolStop: true, ToolUseID: "t1", ToolName: "bash", ToolInput: `{invalid`},
		{Type: "metadataEvent", InputTokens: 10, OutputTokens: 5},
	}
	_, stats := BuildNonStreamingResponse(events, "claude-sonnet-4.6", 200000, nil, 0, 0)
	if !stats.ToolParseError {
		t.Fatal("expected ToolParseError = true for invalid JSON input")
	}
}
